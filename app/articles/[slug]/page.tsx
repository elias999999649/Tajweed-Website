...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const article = getArticleBySlug(slug);
  if (!article) notFound();
  const relatedRules = article.relatedRuleIds.map((id) => tajweedTopics.find((topic) => topic.id === id)).filter(Boolean);
