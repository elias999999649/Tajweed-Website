...
export default function StartHerePage() {
  ...
  return <><SiteHeader /><main><section className="library-hero"><div className="container"><p className="eyebrow">Start here</p><h1>Where should I start?</h1><p className="hero-lede">Begin with the sound foundations, then move through the rule families in the order they depend on one another.</p></div></section><section className="section"><div className="container start-here-list">{stages.map((stage, index) => { const firstRule = stage.rules.map((id) => tajweedRules.find((rule) => rule.id === id)).find(Boolean); return <article className="start-here-step" key={stage.id}><div className="start-here-icon">{icons[index]}</div><div><span className="level-index">STEP {index + 1}</span><h2>{stage.title}</h2><p>{stage.summary}</p><a className="text-link" href={firstRule ? `/tajweed/${firstRule.slug}` : "/tajweed"}>Begin this stage <ArrowRight size={15} /></a></div></article>; })}</div></section><section className="section soft"><div className="container start-here-note"><Headphones size={22} /><div><h2>Listening is part of learning</h2><p>Written explanations help you recognise a rule, but pronunciation, timing, and articulation develop through listening, repetition, and guidance from a qualified Tajweed teacher.</p><a className="text-link" href="/practice">Go to practice <ArrowRight size={15} /></a></div></div></section></main><Footer /></>;
}
