...
export default async function TajweedRulePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><SiteHeader /><main className="rule-page"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} />
    ...
    <div className="container rule-layout"><aside className="rule-sidebar"><nav aria-label="On this page"><strong>On this page</strong><a href="#definition">What is it?</a><a href="#occurs">When it occurs</a><a href="#letters">Letters involved</a><a href="#pronunciation">How to pronounce it</a><a href="#examples">Examples and poster</a><a href="#mistakes">Common mistakes</a><a href="#differences">Important differences</a><a href="#practice">Practice</a><a href="#quiz">Quiz</a><a href="#faq">FAQ</a><a href="#related">Related rules</a></nav></aside>
      <article className="rule-content"><section className="quick-summary card"><div className="quick-summary-heading"><Info size={19} /><h2>Quick Facts</h2></div><dl><div><dt>Level</dt><dd>{rule.level}</dd></div><div><dt>Category</dt><dd>{rule.category}</dd></div><div><dt>When it occurs</dt><dd>{rule.whenItOccurs}</dd></div><div><dt>Letters involved</dt><dd lang="ar" dir="rtl">{rule.letters.length ? rule.letters.join(" · ") : "Context-dependent"}</dd></div></dl></section>
        ...
        <section id="examples"><h2>Examples</h2>{rule.category === "Waqf and Ibtida" && <><figure className="symbols-poster"><Image src="/quran-symbols.png" alt="Poster explaining Quran reading and stopping symbols" width={1600} height={850} sizes="(max-width: 800px) 100vw, 760px" loading="eager" /><figcaption>Poster reference: the right side shows common stopping signs; the left side shows other printed reading marks. Use the poster to recognise a sign, then confirm its application in the mushaf and with a qualified teacher.</figcaption></figure><div className="symbols-guide"><strong>How to use this poster</strong><ol><li>Find the mark in your mushaf.</li><li>Read the instruction beside the matching symbol.</li><li>Protect the meaning before you stop or continue.</li><li>Remember that editions can differ; your teacher and your own mushaf take priority.</li></ol></div></>}<p className="muted">The examples below are clearly labelled teaching examples and are not from the Quran. Use them to notice the rule, then confirm your recitation with a qualified teacher.</p><div className="examples-list">{verifiedQuranExamples.map((example) => <QuranExampleCard example={example} key={example.id} />)}{teachingExamples.length > 0 && <div className="teaching-example"><strong>Teaching examples (not Quran quotations):</strong>{teachingExamples.map((example) => <p key={example.id} lang="ar" dir="rtl">{highlightText(example.arabicText, example.highlightedText)} <small dir="ltr">— {example.transliteration} ({example.translation}) — <strong>Not from the Quran</strong></small></p>)}</div>}{!verifiedQuranExamples.length && !teachingExamples.length && <p className="muted">Examples for this lesson will appear after verification of the exact text, reference, and rule annotation.</p>}</div></section>
        ...
        <section id="practice"><h2>Practice</h2><p className="muted">Try to identify the trigger first, say the sound slowly, then listen and repeat. Short, accurate practice is more useful than rushing through many examples.</p><ol className="practice-list">{rule.practiceQuestions.map((exercise) => <li key={exercise}>{exercise}</li>)}</ol></section>
        <section id="quiz">
        ...
        <nav className="rule-pager" aria-label="Lesson order">
          {previousRule ? <a className="pager-prev" href={`/tajweed/${previousRule.slug}`}><small>Previous lesson</small><strong>← {previousRule.name}</strong></a> : <a href="/learn"><small>New here?</small><strong>Start with the learning path</strong></a>}
