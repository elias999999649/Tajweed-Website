...
export function TajweedDirectory() {
  ...
  return <section className="directory-shell" aria-label="Tajweed rules directory">
    <div className="directory-toolbar">
      <div className="directory-mode"><button className={viewMode === "learning" ? "directory-mode-button active" : "directory-mode-button"} onClick={() => setViewMode("learning")}><BookOpen size={16} />Learning order</button><button className={viewMode === "all" ? "directory-mode-button active" : "directory-mode-button"} onClick={() => setViewMode("all")}><ListFilter size={16} />All Rules</button></div>
      <span className="directory-count">{matchingRules.length} of {tajweedRules.length} rules</span>
    </div>
    <div className="directory-filters"><div className="filter-heading"><SlidersHorizontal size={16} />Filter the directory</div><label>Category<select value={category} onChange={(event) => setCategory(event.target.value)}><option value="all">All categories</option>{categories.map((item) => <option value={item} key={item}>{item}</option>)}</select></label><label>Learning level<select value={level} onChange={(event) => setLevel(event.target.value as "all" | DifficultyLevel)}><option value="all">All levels</option>{levels.map((item) => <option value={item} key={item}>{item}</option>)}</select></label><label>Difficulty<select value={difficulty} onChange={(event) => setDifficulty(event.target.value as DifficultyFilter)}>{Object.entries(difficultyLabels).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label><label>Prerequisites<select value={prerequisite} onChange={(event) => setPrerequisite(event.target.value)}><option value="all">Any prerequisite</option><option value="none">No prerequisites</option>{prerequisiteOptions.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label></div>
    {viewMode === "learning" && <div className="directory-intro"><Filter size={17} /><div><strong>Here is what you should learn first.</strong><span>The stages follow the stored curriculum order, from sound foundations to advanced recitation topics.</span></div></div>}
    ...
    {!groups.length && <div className="directory-empty"><strong>No rules match these filters.</strong><span>Clear one or more filters to continue exploring the published directory.</span></div>}
  </section>;
}
